...
def email_lookup():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if is_disposable:
                print(f"{Colors.WHITE}{'Disposable:':<15}{'⚠️ YES (Temp Mail)':>85}{Colors.RESET}")
            else:
                print(f"{Colors.WHITE}{'Disposable:':<15}{'✅ NO':>85}{Colors.RESET}")
            
            is_role = data.get('is_role', False)
            if is_role:
                print(f"{Colors.WHITE}{'Role:':<15}{'⚠️ YES (info@, admin@)':>85}{Colors.RESET}")
            else:
            ...
            print(f"{Colors.WHITE}{'Search Links:':<15}{'':>85}{Colors.RESET}")
            print(f"{Colors.WHITE}{'Google:':<15}{f'https://www.google.com/search?q={email}':>85}{Colors.RESET}")
            print(f"{Colors.WHITE}{'LinkedIn:':<15}{f'https://www.linkedin.com/search/results/people/?keywords={email}':>85}{Colors.RESET}")
            print(f"{Colors.WHITE}{'Twitter:':<15}{f'https://twitter.com/search?q={email}':>85}{Colors.RESET}")
            print(f"{Colors.WHITE}{'Facebook:':<15}{f'https://www.facebook.com/search/top?q={email}':>85}{Colors.RESET}")
